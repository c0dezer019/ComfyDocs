      ...
      <section className="glass-card rounded-3xl p-8 relative overflow-hidden group">
        ...
        <div className="flex items-center gap-4 mb-8">
          <div className="p-3 bg-indigo-500/10 rounded-2xl text-indigo-400 ring-1 ring-indigo-500/20">
             {aiStatus === 'loading' ? <Loader2 size={24} className="animate-spin" /> : <Layers size={24} />}
          </div>
          <div>
                           ...
                           return (
                             <li key={issue.id} className="p-5 bg-indigo-500/5 border border-indigo-500/10 rounded-2xl animate-in slide-in-from-right-4 duration-500">
                                <div className="flex items-center gap-3 mb-3">
                                    <div className={`w-1.5 h-1.5 rounded-full ${issue.severity === 'Note' ? 'bg-blue-400' : 'bg-rose-400'}`} />
                                    <span className={`text-[10px] font-black uppercase tracking-widest ${issue.severity === 'Note' ? 'text-blue-300' : 'text-rose-300'}`}>
                                        {issue.type}
                                    </span>
                                </div>
        ...
        <section className="glass-card rounded-3xl p-8 relative overflow-hidden">
            ...
            <div className="space-y-6">
                ...
                <div className="relative mt-8 group/input">
                    <input type="text" className="w-full bg-black/40 border border-white/10 rounded-2xl py-5 pl-8 pr-16 text-sm text-white placeholder:text-slate-600 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500/50 outline-none transition-all shadow-inner" placeholder="Analyze specific elements of the generation..." value={question} onChange={(e) => setQuestion(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && !isAsking && handleSubmitQuestion()} disabled={isAsking} />
                    <button onClick={handleSubmitQuestion} disabled={!question.trim() || isAsking} className="absolute right-3 top-1/2 -translate-y-1/2 p-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl shadow-xl transition-all hover:scale-105 disabled:opacity-30 disabled:hover:scale-100">
                        <Send size={20} />
                    </button>
                </div>
            </div>
