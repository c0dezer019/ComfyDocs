    ...
    <div className="flex flex-col items-center">
      ...
      <div className="print-only bg-white text-slate-900 w-full max-w-[210mm] min-h-[297mm] p-[15mm] shadow-2xl rounded-sm mx-auto relative">
         ...
         <div className="border-b-2 border-slate-900 pb-4 mb-6 flex justify-between items-end">
             <div>
                 <h1 className="text-2xl font-serif font-bold text-slate-900 mb-1">Forensic Audit Report</h1>
                 <p className="text-[10px] uppercase tracking-widest text-slate-500 font-bold">ComfyDocs Automated Analysis</p>
             </div>
             <div className="text-right">
                 <div className="text-xs font-mono text-slate-500 mb-1">{new Date().toLocaleDateString()}</div>
             ...
             <div className="col-span-8 flex flex-col gap-4">
                 ...
                 <div className="flex gap-3">
                     <div className="flex-1 p-3 bg-slate-50 border border-slate-200 rounded">
                         ...
                         <div className={`text-2xl font-black ${data.qualityAnalysis?.overallScore && data.qualityAnalysis.overallScore >= 8 ? 'text-emerald-600' : 'text-slate-900'}`}>
                             {data.qualityAnalysis?.overallScore || 'N/A'}<span className="text-xs text-slate-400 font-normal">/10</span>
                         </div>
                     </div>
                     <div className="flex-1 p-3 bg-slate-50 border border-slate-200 rounded">
                         <div className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-1">Adherence</div>
                         <div className="text-2xl font-black text-slate-900">
                             {data.promptAnalysis?.adherenceScore || 'N/A'}<span className="text-xs text-slate-400 font-normal">/10</span>
                         </div>
                     </div>
                 </div>
                 ...
                 {/* Executive Summary */}
                 <div>
                     <h3 className="font-serif font-bold text-base mb-2 border-b border-slate-200 pb-1">Executive Summary</h3>
                     <div className="space-y-1.5">
                        {data.sceneOverview.map((s, i) => (
                            <div key={i} className="text-xs leading-relaxed">
                                <span className="font-bold text-slate-700 uppercase text-[10px] tracking-wide mr-2">{s.category}:</span>
                                {s.details}
                            </div>
                        ))}
                     </div>
         ...
         <div className="mb-8 avoid-break">
             <h3 className="font-serif font-bold text-base mb-3 border-b border-slate-200 pb-1">Technical Profile</h3>
         ...
         {/* Issues Table */}
         <div className="mb-8">
             <h3 className="font-serif font-bold text-base mb-3 border-b border-slate-200 pb-1">Quality Assurance Log</h3>
                 ...
                 <tbody>
                     {data.qualityAnalysis?.issues.map((issue, idx) => (
                         <tr key={idx} className="border-b border-slate-100 break-inside-avoid">
                             ...
                             <td className={`py-2 px-2 text-right font-mono font-medium align-top ${issue.score > 0 ? 'text-red-500' : 'text-slate-400'}`}>
         ...
         <div className="avoid-break">
            <h3 className="font-serif font-bold text-base mb-3 border-b border-slate-200 pb-1">Prompt Engineering Audit</h3>
            ...
            {data.promptAnalysis?.improvements && data.promptAnalysis.improvements.length > 0 && (
                <div>
                    <div className="text-[10px] font-bold uppercase text-slate-500 mb-1.5">Recommended Improvements</div>
