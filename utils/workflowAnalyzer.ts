...
export const analyzeWorkflowLocally = (workflow: any): SceneDocumentation => {
  ...
  const resolveTextFromNode = (node: any, visited = new Set<number>()): string | null => {
      ...
      if (node.widgets_values && Array.isArray(node.widgets_values)) {
          for (const val of node.widgets_values) {
              if (typeof val === 'string' && val.trim().length > 0) {
                   ...
                   if (['fixed', 'increment', 'decrement', 'randomized', 'nearest-exact', 'bilinear', 'enable', 'disable'].includes(val.toLowerCase())) continue;
                   return val;
              }
          }
      }
      ...
      if (node.inputs) {
          ...
          const textInputNames = ['text', 'string', 'text_g', 'text_l', 'prompt', 'text_positive', 'text_negative'];
  ...
  const samplers = nodes.filter((n: any) => {
  ...
  if (kSampler) {
      ...
      widgets.forEach((w: any) => {
          ...
          if (typeof w === 'string') {
               ...
               if (['euler', 'dpm', 'ddim', 'uni_pc', 'lms'].some(t => s.includes(t))) result.parameters.sampler = w;
               if (['normal', 'karras', 'exponential', 'simple', 'sgm_uniform'].some(t => s.includes(t))) result.parameters.scheduler = w;
          }
      });
      ...
      if (modelNode) {
          ...
          while(curr && depth < 10) {
              ...
              if (t.includes('checkpoint') || t.includes('loader')) {
                  const modelName = curr.widgets_values?.find((v: any) => typeof v === 'string' && v.match(/\.(safetensors|ckpt|sft)$/i));
