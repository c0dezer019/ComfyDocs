  ...
  const responseSchema = {
    ...
    properties: {
      sceneOverview: {
        type: Type.ARRAY,
        description:
          "List of visual attributes. Categories should be 'Subject', 'Lighting', 'Composition', 'Style', etc.",
        items: {
          type: Type.OBJECT,
          properties: {
      ...
      qualityAnalysis: {
        ...
        properties: {
          overallScore: {
            type: Type.NUMBER,
            description: 'Strictly calculated as: 10 - Sum(issues.score).',
          },
          issues: {
            type: Type.ARRAY,
            items: {
              ...
              properties: {
                ...
                box_2d: {
                  ...
                  description: 'Optional [ymin, xmin, ymax, xmax] if issue is localized',
                },
...
export const runConsensusQualityAnalysis = async (
  ...
  const passPromises = Array(passCount)
    .fill(0)
    .map(() =>
      ai.models.generateContent({
        ...
        contents: {
          parts: [
            ...
            {
              text: 'List all visual quality issues, artifacts, or anatomical errors in this image.',
            },
          ],
        },
        config: {
...
export const askQuestion = async (
  ...
  updates?: { critique?: string; newImprovements?: string[] };
}> => {
  const apiKey = getApiKey();
  if (!apiKey) throw new Error('API_KEY_NOT_FOUND');
        ...
        items: {
          ...
          properties: {
            ...
            style: {
              ...
              enum: ['box', 'paint'],
              description:
                "Use 'box' for discrete objects (hands, faces) and 'paint' for surface issues (texture, lighting).",
            },
            box_2d: {
              type: Type.ARRAY,
              items: { type: Type.NUMBER },
