...
export const DocumentationViewer: React.FC<DocumentationViewerProps> = ({
  ...
  const handleSaveNote = async (issue: QualityIssue) => {
    ...
    const newIssues = data.qualityAnalysis.issues.map((i) =>
  ...
  const handleGenerateIssues = async () => {
    if (!data.userSceneNotes || data.userSceneNotes.length === 0 || !onGenerateIssuesFromNotes)
      return;
  ...
  const handleDeleteIssue = (index: number) => {
    ...
    onUpdateData({
      ...data,
      qualityAnalysis: { ...data.qualityAnalysis, issues: newIssues, overallScore: newScore },
    });
  };
  ...
  const handleDeleteImprovement = (index: number) => {
    ...
    onUpdateData({
      ...data,
      promptAnalysis: { ...data.promptAnalysis, improvements: newImprovements },
    });
  };
          ...
          <div className="space-y-6">
            ...
            <div className="relative mt-8 group/input">
              <input
                ...
                onKeyDown={(e) =>
                  e.key === 'Enter' && !isAsking && aiStatus === 'complete' && handleSubmitQuestion()
                }
      ...
      <section className="glass-card rounded-3xl p-8 relative overflow-hidden group">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            {isOffline && (
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mt-1 block">
                Local Recovery Mode
              </span>
            )}
          </div>
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                <div className="flex flex-row items-center gap-4 bg-black/40 px-5 py-2.5 rounded-2xl border border-white/5 shadow-inner">
                  ...
                  <span className={`text-xl font-black ${getScoreColor(data.qualityAnalysis.overallScore)}`}>
                    {data.qualityAnalysis.overallScore}
                    <span className="text-xs text-slate-600 ml-0.5">/10</span>
                  </span>
                </div>
              </div>
            )}
            ...
            data.qualityAnalysis && (
              <div className="space-y-4">
                {filteredIssues.map((issue, idx) => (
                  <div
                    key={issue.id || idx}
                    className="bg-white/5 rounded-2xl border border-white/5 p-6 group/issue relative transition-all hover:bg-white/[0.08] hover:border-white/10"
                  >
                      ...
                      <div className="space-y-4">
                        <div className="flex gap-4">
                          <select
                            ...
                            onChange={(e) =>
                              setEditingIssue({ ...editingIssue, severity: e.target.value as any })
                            }
                          >
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setEditingIssue({ ...editingIssue, type: e.target.value })
                            }
                          />
                        </div>
                        <textarea
                        ...
                        <div className="flex justify-end gap-3">
                          ...
                          <button
                            onClick={() => handleSaveIssue(idx, editingIssue)}
                      ...
                      <div className="flex flex-col gap-4">
                        <div className="flex items-start gap-6">
                          ...
                          <div className="absolute top-6 right-6 flex gap-1 opacity-0 group-hover/issue:opacity-100 transition-opacity">
                            ...
                            <button
                              onClick={() => handleStartEditing(idx, issue)}
                          ...
                          <div className="ml-10 p-5 bg-black/40 border-l-4 border-indigo-500/50 rounded-r-2xl text-sm animate-in slide-in-from-top-2">
                            ...
                            {editingNoteId === issue.id ? (
                              ...
                              <p className="italic text-slate-400 leading-relaxed">
                                {issue.userNotes || 'No context added yet.'}
                              </p>
                            )}
                          </div>
                        )}
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="bg-black/40 px-5 py-2.5 rounded-2xl border border-white/5 shadow-inner flex items-center gap-4">
                  ...
                  <span className={`text-xl font-black ${getScoreColor(data.promptAnalysis.adherenceScore)}`}>
                    {data.promptAnalysis.adherenceScore}
                    <span className="text-xs text-slate-600 ml-0.5">/10</span>
                  </span>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  {editingPromptAnalysis ? (
                    <div className="space-y-4">
                      ...
                      <div className="flex justify-end gap-3">
                        <button
                          onClick={() => setEditingPromptAnalysis(false)}
                          className="text-[10px] font-black text-slate-500 uppercase px-4"
                        >
                          Cancel
                        </button>
                        <button
                          onClick={handleSavePromptAnalysis}
          ...
          {Object.entries(data.parameters).map(
            ([key, value]) =>
              (value || value === 0) && (
                <div
                  key={key}
                  className="bg-black/30 p-5 rounded-2xl border border-white/5 shadow-inner group/param hover:border-amber-500/30 transition-all"
                >
      ...
      <section className="glass-card rounded-3xl p-8 relative">
        <div className="flex items-center gap-4 mb-8 pb-6 border-b border-white/5">
          ...
          <div>
            ...
            <div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mt-1">
              Interactive node reconstruction
            </div>
          </div>
        </div>
